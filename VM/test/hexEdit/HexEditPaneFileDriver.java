...
public class HexEditPaneFileDriver {
	...
	private HexEditPanelFile hexEditPanel;
	private HexEditPanelFile hexEditPanel_1;

	private String strReadName;
	private File fileRead;
	private FileInputStream streamRead;
	private FileChannel channelRead;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					HexEditPaneFileDriver window = new HexEditPaneFileDriver();
	...
	private void doBtnOne() {
		File fileRead = new File(strReadName);
		if (!fileRead.exists()){
			System.out.printf("[doBtnOne] does not exists: %s%n", strReadName);
			return;
		}//if
		System.out.printf("[doBtnOne] read File: %s%n", strReadName);
		hexEditPanel.loadData(fileRead);
	...
	private void doFileOpen() {
		...
		if (!(fileChooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION)) {
			System.out.printf("[doFileOpen] did not approve of selection%n");
			return;
		} // if
		fileRead = fileChooser.getSelectedFile();
		strReadName = fileRead.getAbsolutePath();
		lblFileName.setText(strReadName);
		// System.out.printf("[] fileName = %s%n", fileRead.getAbsolutePath());
	...
	private void appClose() {
		Preferences myPrefs = Preferences.userNodeForPackage(HexEditPaneFileDriver.class);
	...
	private void appInit() {
		...
		strReadName = (myPrefs.get("ReadFileName", "C:\\Users\\admin\\Documents\\biosTestForNio.asm"));
		lblFileName.setText(strReadName);
		myPrefs = null;
	...
	private void initialize() {
		...
		frmTemplate.setTitle("HexEditFileFileDriver");
		...
		hexEditPanel = new HexEditPanelFile();
		...
		hexEditPanel_1 = new HexEditPanelFile();
