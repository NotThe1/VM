...
public class ArithmeticUnit implements IArithmeticUnit {
	...
	public byte addWithCarry(byte operand1, byte operand2) {
		...
		if (carryFlagIn) {
			...
			auxCarryFromIncrement = ccr.isAuxilaryCarryFlagSet(); // Aux carry from increment
		}// if carryFlagIn

		byte result = (byte) add((int) operand1, (int) operand2, MASK_BYTE);
		boolean carryFlagOut = ccr.isCarryFlagSet() | carryFromIncrement;// carry is true if either flag is set
		boolean auxCarryFlagOut = ccr.isAuxilaryCarryFlagSet() | auxCarryFromIncrement;// Aux carry is true if either
																						// flag is set
		ccr.setAuxilaryCarryFlag(auxCarryFlagOut);
